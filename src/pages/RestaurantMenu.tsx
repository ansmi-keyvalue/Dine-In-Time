...
export default function RestaurantMenu({ restuarantId }: RestaurantMenuProps) {
  const [selectedCategory, setSelectedCategory] = useState<RestaurantMenuCategories>(RestaurantMenuCategories.Recommended);
      ...
      <div className="mt-14 pb-36">
        <section className="flex flex-col">
          ...
          <p className="text-zinc-900 text-3xl w-full mt-3 max-md:max-w-full">
            ...
            since the 1500s, when an unknown printer took a galley Ipsum.
          </p>
        ...
        <div className="flex mt-10 gap-2">
            ...
            {menuCategories.map((item) => (
              <button
                key={item}
                className={`flex text-black text-3xl font-semibold ${selectedCategory === item ? 'text-orange-600' : ''}`}
